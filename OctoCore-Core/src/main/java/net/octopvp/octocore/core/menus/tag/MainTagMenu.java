...
public class MainTagMenu extends Menu<Gui> {
    ...
    public Gui createGui(Player player) {
        return Gui.gui().title("Tags").rows(3).create();
    }
    ...
    public GuiItem viewAll() {
        return ItemBuilder.from(Material.GOLDEN_CARROT).name(CC.GOLD + "View All Tags").lore(CC.SEPARATOR, CC.AQUA + "Click here to view all tags!", CC.SEPARATOR).asGuiItem(event -> {
