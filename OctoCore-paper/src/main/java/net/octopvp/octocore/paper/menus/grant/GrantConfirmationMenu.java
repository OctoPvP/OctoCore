...
public class GrantConfirmationMenu extends Menu {
    ...
    private class YesButton extends Button{
        ...
        public void onClick(Player player, int slot, ClickType clickType) {
            ...
            Tasks.runAsync(()->{
                PlayerData targetData = PlayerManager.getData(grantProcedure.getTargetData().getUuid());
                if (targetData == null){
                    targetData = PlayerManager.getProfileFromDB(grantProcedure.getTargetData().getUuid());
                }
                ...
                if (globalPlayer != null){
                    OctoCore.getInstance().getRedisData().write(JedisAction.GRANTS_UPDATE,new JsonChain().addProperty("name",targetData.getName()).addProperty("add",true).addProperty("tochange",OctoCore.getGson().toJson(grant)).get());
                }else{
