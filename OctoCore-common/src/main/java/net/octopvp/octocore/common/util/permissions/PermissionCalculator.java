...
public class PermissionCalculator {
    ...
    public static PermissionResult hasPermissionResult(String perm, Collection<Node> permissions, String server) {
        ...
        if (b.getReason() == PermissionReason.NOT_SET && allPerms)
            b = new PermissionResult(perm, "*", PermissionReason.ALLOWED_WILDCARD);
        else if (b.getReason() == PermissionReason.NOT_SET && allPermsNegated)
            b = new PermissionResult(perm, "*", PermissionReason.NEGATED_WILDCARD);
        return b;
    }
        ...
        map.forEach((permission, allowed) -> {
            if (perm.toLowerCase().startsWith(permission.toLowerCase())) {
                ...
                if (allowed)
                    ...
                    b.set(new PermissionResult(perm, permission + SUB_WILDCARD, PermissionReason.NEGATED_SUB_WILDCARD));
